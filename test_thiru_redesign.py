...
2. Run segmentation
3. Verify 5-panel display (Input, Mitochondria, Membrane, Ribbon, Vesicles)
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        try:
            page.wait_for_function(
                """() => {
                    ...
                    return imgs.length >= 5 && imgs[0].src && imgs[0].src.length > 100;
                }""",
                timeout=SEG_TIMEOUT,
            )
            time.sleep(5)
        ...
        # ===================== 5-PANEL DISPLAY =====================
        log("=" * 60, results)
        log("TEST 4: Five-panel display (Input, Mito, Membrane, Ribbon, Vesicles)", results)
        ...
        has_vesicles = "Vesicles" in page_html
        ...
        log(f"  Labels: Input={has_input}, Mitochondria={has_mito}, Membrane={has_membrane}, Ribbon={has_ribbon}, Vesicles={has_vesicles}", results)

        if panel_count >= 5 and has_input and has_mito and has_membrane and has_ribbon and has_vesicles:
            log("  PASS: Five-panel display present", results)
            passed += 1
        else:
            log("  FAIL: Expected 5 panels with correct labels", results)
            failed += 1
        ...
        expected_files = [
            ...
            "overlay_Synaptic_Vesicles",
            "mask_Mitochondria",
            "mask_Presynaptic_Membrane",
            "mask_Synaptic_Ribbon",
            "mask_Synaptic_Vesicles",
            "metrics.csv",
        ]
        ...
        if len(found_f) >= 8 and len(found_unwanted) == 0:
